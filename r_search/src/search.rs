...
impl RSearch {
    ...
    pub fn kw_add(&mut self, kw: RSearchKeyword) {
        self.kws.push(kw);
    }
    ...
    pub fn update(&mut self, from: usize) {
        ...
        if !self.kws.is_empty() {
            self.binparse_update(from);
        }
    }
    fn binparse_update(&mut self, from: usize) -> u64 {
        ...
        for i in 0..kws_len {
            ...
            while offset < len && qs_find(&self.kws[i], offset, &self.buf, &mut match_pos) {
                self.add_hit(i, from + match_pos);
                offset = match_pos + self.kws[i].len();
            }
        }
        return 0;
    }
    fn add_hit(&mut self, kw_idx: usize, addr: usize) {
        ...
        match self.callback {
            Some(cb) => cb(&self.kws[kw_idx], addr, &self.buf),
            _ => (),
        }
        let hit = RSearchHit {
            kw_idx: kw_idx,
