...
fn do_hash_hexprint(c: &[u8], little_endian: bool) {
    if little_endian {
        ...
        let mut i = 0;
        while i < c.len() {
            print!("{:02x}{:02x}{:02x}{:02x}", c[i + 3], c[i + 2], c[i + 1], c[i]);
            i += 4;
        }
    } else {
        for i in c {
...
fn do_hash(file: &str, algo: &str, io: &c_void, mut bsize: usize, compare: &[u8], mut state: &mut State, s: &r_hash::RHashSeed) {
    ...
    if state.incremental {
        ...
        while i < r_hash::R_HASH_ALL {
            ...
            if hashbit != 0 {
                ...
                while j < state.to {
                    let nsize = if j + bsize > state.to { state.to - j } else { bsize };
...
fn encrypt_or_decrypt_file(algo: &str, is_decryption: bool, file: &str, iv: &[u8], s: &r_hash::RHashSeed) {
...
fn encrypt_or_decrypt(algo: &str, is_decryption: bool, buf: &[u8], iv: &[u8], s: &r_hash::RHashSeed) {
...
fn argument_parser() -> Matches {
    ...
    let matches;
    opts.optopt("a", "", "comma separated list of algorithms (default is 'sha256')", "algo");
    opts.optflag("B", "", "show per-block hash");
    opts.optopt("b", "", "specify the size of the block (instead of full file)", "bsize");
    opts.optopt("c", "", "compare with this hash", "hash");
    opts.optopt("d", "", "decrypt. Use -S to set key and -I to set IV", "algo");
    opts.optflag("E", "", "use as little endian");
    opts.optopt("e", "", "encrypt. Use -S to set key and -I to set IV", "algo");
    opts.optopt("f", "", "start hashing at given address", "from");
    opts.optflag("h", "", "print this help message");
    opts.optopt("I", "", "use give initialization vector (IV) (hexa or s:string)", "iv");
    ...
    opts.optopt("x", "", "hash this hexpair string instead of files", "hexpair");
    ...
    if args.len() == 1 {
        ...
        let help = format!(
            "Usage: {} [-rBhlkvje] [-b S] [-a A] [-c H] [-e A] \
             [-s S] [-f O] [-t O] [file] ...",
            program
        );
        r_print::report(&help);
...
fn main() {
    ...
    if !compare_str.is_empty() {
        let algobit: u64;
        if &encrypt == "base64" || &encrypt == "base91" || &decrypt == "base64" || &decrypt == "base91" {
            r_print::report(
                "Option -c incompatible with -E base64, -E base91, -D base64 or \
                 -D base91 options.",
            );
        }
        ...
        if compare_bin.len() != r_hash::size(algobit) {
            let err_msg = format!(
                "rahash2: Given -c hash has {} bytes but the \
