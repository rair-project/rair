...
fn argument_parser() -> Matches {
    ...
    opts.optopt("e", "", "search for regular expression string matches", "regex");
    ...
    opts.optopt("M", "", "set a binary mask to be applied on keywords", "str");
    opts.optflag("r", "", "print as radare2 flag commands");
    opts.optmulti("s", "", "search for a specific string (can be used multiple times)", "str");
    opts.optopt("t", "", "stop search at address 'to'", "to");
    opts.optflag("v", "", "print version and exit");
    opts.optmulti("x", "", "search for hexpair string (can be used multiple times)", "hex");
    ...
    if env::args().len() == 1 {
        ...
        let help = format!(
            "Usage: {} [-mXnzZhv] [-a align] [-b sz] [-f/t from/\
...
fn rafind_process(file: &str, list: &[SearchEntry], mut state: State) {
    ...
    for entry in list {
        match entry.mode {
            ...
            SearchMode::Regex => match rs.regex_add(&entry.entry) {
                Err(y) => r_print::report(&y.to_string()),
                _ => (),
            },
            _ => unimplemented!(),
