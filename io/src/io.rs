...
// The expectation must be on the module: the lint fires on the inherent
// `serialize`/`deserialize` methods generated by the remote derive, and
// derive-generated impls are siblings of `RIO` rather than its children.
#![expect(
    clippy::same_name_method,
    reason = "serde remote-derive pattern generates inherent serialize/deserialize used by the manual trait impls"
)]
...
impl RIO {
    ...
    /// # Errors
    ///
    /// Returns [`IoError::HndlNotFoundError`] if `hndl` does not describe a
    /// currently open file.
    ///
    /// # Example
    ///
    ...
    /// # Errors
    ///
    /// Returns [`IoError::AddressNotFound`] if the physical range starting at
    /// `paddr` with length `size` is not fully backed by open files, and
    /// [`IoError::AddressesOverlapError`] if the requested virtual range
    /// overlaps an already existing memory mapping.
    ...
    /// # Errors
    ///
    /// Returns [`IoError::IoPluginNotFoundError`] if no plugin accepts `uri`.
    /// Otherwise errors produced by the accepting plugin (typically
    /// [`IoError::Parse`] or [`IoError::Custom`]) are propagated when the
    /// file itself cannot be opened with the requested `flags`.
    ///
    /// # Example
    ///
    /// ```
    ...
    ///
    /// Returns [`IoError::IoPluginNotFoundError`] if no plugin accepts `uri`,
    /// and [`IoError::AddressesOverlapError`] if the file, once loaded at
    /// physical address `at`, would overlap an already open file. Errors
    /// produced by the accepting plugin (typically [`IoError::Parse`] or
    /// [`IoError::Custom`]) are propagated when the file itself cannot be
    /// opened with the requested `flags`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///
    /// Returns [`IoError::AddressNotFound`] if the physical range starting at
    /// `paddr` with the length of `buf` is not fully backed by open files.
    /// Errors reported by the underlying plugins (typically [`IoError::Parse`])
    /// are propagated when reading from the backing files fails.
    ///
    /// # Panics
    ///
    /// Panics only if the internal descriptor bookkeeping is corrupted, i.e. a
    /// handle reported as backing the requested range can no longer be
    /// resolved. This is considered a bug.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// # Errors
    ///
    /// Returns an [`IoError`] (typically [`IoError::Parse`]) only when one of
    /// the underlying plugins fails to read from its backing file; unbacked
    /// parts of the requested range are silently left out of the result.
    ///
    /// # Panics
    ///
    /// Panics only if the internal descriptor bookkeeping is corrupted, i.e. a
    /// handle reported as backing part of the requested range can no longer be
    /// resolved. This is considered a bug.
    ///
    /// # Example
    ///
    ...
    /// `paddr` with the length of `buf` is not fully backed by open files.
    /// Errors reported by the underlying plugins (typically [`IoError::Parse`],
    /// for example when a file is not writable) are propagated when writing to
    /// the backing files fails.
    ///
    /// # Panics
    ///
    /// Panics only if the internal descriptor bookkeeping is corrupted, i.e. a
    ...
    /// # Errors
    ///
    /// Returns [`IoError::AddressNotFound`] if the virtual range starting at
    /// `vaddr` with length `size` is not fully covered by existing memory
    /// mappings.
    ...
    ///
    /// Returns [`IoError::AddressNotFound`] if the virtual range starting at
    /// `vaddr` with the length of `buf` is not fully covered by existing
    /// memory mappings. Errors reported by the underlying plugins (typically
    /// [`IoError::Parse`]) are propagated when reading from the backing files
    /// fails.
    ...
    ///
    /// Returns an [`IoError`] (typically [`IoError::Parse`]) only when one of
    /// the underlying plugins fails to read from its backing file; unmapped
    /// parts of the requested range are silently left out of the result.
    ...
    /// `vaddr` with the length of `buf` is not fully covered by existing
    /// memory mappings. Errors reported by the underlying plugins (typically
    /// [`IoError::Parse`], for example when a file is not writable) are
    /// propagated when writing to the backing files fails.
